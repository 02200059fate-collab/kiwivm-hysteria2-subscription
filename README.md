...
- 抖音先匹配 `DouYin` 和完整 `ByteDance` 规则，再用精简的 `ChinaIPsBGP` 兜住 HTTPDNS 返回的国内 CDN IP；TikTok 不包含在这些国内直连规则中，仍按国外流量处理。
- iPhone 实测播放抖音两分钟约有 60.7 MB 下载走直连，代理下载仅约 0.16 MB；该结果用于验证分流方向，不代表固定带宽或速度。
